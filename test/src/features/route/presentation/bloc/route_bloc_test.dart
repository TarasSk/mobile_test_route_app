...
void main() {
  ...
  group('RouteBloc', () {
    ...
    test('emits RouteState.loading and then RouteState.loaded on successful route load', () async {
      ...
      when(() => mockRouteUseCase.call(routeParams)).thenAnswer((_) async => mockRouteEntity);

      final weatherParams = WeatherUseCaseParams(lat: 37.7749, lng: -122.4194);

      when(() => mockWeatherUseCase.call(weatherParams)).thenAnswer((_) async => mockWeatherEntity);
